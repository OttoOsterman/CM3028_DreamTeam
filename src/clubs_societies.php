...
<div id=clublist>
	...
	while ($row = $result->fetch_array()) {
        if (isset($row['photo_path'])) {
            ...
            echo("
			<section><img id='clubImage' src='images/default.jpg'><h1 id='clubName'><a href='club/{$row['club_id']}'>{$row['name']}</a></h1><div id='clubGenre'>{$row['genre']}</div><div id='clubDesc'>{$row['description']}</div></section>
            ");
        }
	}
	?>
